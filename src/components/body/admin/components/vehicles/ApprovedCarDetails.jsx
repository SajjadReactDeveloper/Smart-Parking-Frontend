...
export default function ApprovedCarDetails() {
  const [confirmDialog, setConfirmDialog] = useState({isOpen: false, title: '', subTitle: ''})
    ...
    const approveCar = async(id, regNo, brand, model, color, ownerName, ownerId, ownerAvatar, carImage) => {
        try {
          const res = await axios.post(`/car/approveCar`, {
            id,
            regNo,
            brand,
            model,
  ...
  return (
    <div className="container mt-5">
      ...
      <CardActions>
      <Button onClick={() => approveCar(car._id, car.regNo, car.brand, car.model, car.color, car.ownerName, car.ownerId, car.ownerAvatar, car.carImage)}
        style={{float: 'right'}} variant="outlined" color='success'>
          Accept
        </Button>
        <Button onClick={() => {
          deleteUser(car._id)
        }} style={{float: 'right', marginLeft: 'auto'}} variant="outlined" color='error'>
          Rejects
        </Button>
      ...
      >
        ...
        >
          ...
          <div className="" style = {{marginTop: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
            <Button
              onClick={() =>
                approveCar(
                  car._id,
