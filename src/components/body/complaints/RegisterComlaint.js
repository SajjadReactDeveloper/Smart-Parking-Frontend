...
function RegisterComlaint() {
  ...
  const [values, setUser] = React.useState(initialState);
    const {category, subCategory, type, title, description, err, success} = values;
    ...
    const handleSubmit = async(e) => {
        ...
        try {
            const res = await axios.post('/complaint/addComplaint', {
                category, subCategory, type, title, description, userId:auth.user._id
            })
            alert(res.data.msg);
          ...
          <div className="row">
            <div className="col-10">
              ...
              <form action="" className="mainForm" onSubmit={handleSubmit}>
                ...
                <br />
                <input type="text" name="title" placeholder="Complaint Title" onChange={handleChangeInput} className="form-control" style={{marginBottom: 10}} />
                <label htmlFor="">
